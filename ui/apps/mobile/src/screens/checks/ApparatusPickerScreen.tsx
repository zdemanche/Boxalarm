  ...
  useEffect(() => {
    ...
    repository
      ...
      .then((result) => {
        if (cancelled) return;
        setApparatus(result);
        setIsOffline(repository.isApparatusFallback?.() ?? false);
      })
      .catch((error: unknown) => {
        if (cancelled) return;
        setLoadError(
          error instanceof ApiError
            ? 'You do not have access to apparatus. Sign out and back in, or contact your department administrator.'
            : 'Apparatus could not be loaded. Check your connection and try again.',
        );
      });
    return () => {
      cancelled = true;
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: tokens.background }}>
      {isOffline ? (
        ...
        >
          Offline — showing last known apparatus.
        </Text>
      ) : null}
      {loadError ? (
        <Text
