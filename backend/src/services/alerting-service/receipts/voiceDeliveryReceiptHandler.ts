...
interface VoiceReceiptWebhookBody {
...
export function parseVoiceReceiptWebhookBody(
  raw: string | undefined | null,
): VoiceReceiptWebhookBody {
  let parsed: unknown;
  try {
...
export const handler = async (event: APIGatewayProxyEventV2): Promise<APIGatewayProxyResultV2> => {
  ...
  const secret =
    event.headers?.['x-voice-provider-secret'] ?? event.headers?.['X-Voice-Provider-Secret'];
  const webhookSecret = process.env.VOICE_PROVIDER_WEBHOOK_SECRET;
  if (!webhookSecret || !verifyVendorSecret(secret, webhookSecret)) {
    emitOutcomeMetric(METRIC_NAMESPACE, 'VoiceReceiptRejected', 'Unauthorized');
    return unauthorizedVendorProblem(
      traceId,
      'Voice provider webhook secret is missing or invalid.',
    );
  }

  let body: VoiceReceiptWebhookBody;
  try {
    body = parseVoiceReceiptWebhookBody(event.body);
  } catch (error) {
    emitOutcomeMetric(METRIC_NAMESPACE, 'VoiceReceiptRejected', 'ValidationFailed');
  ...
  try {
    ...
    if (result.outcome === 'not_found') {
      emitOutcomeMetric(METRIC_NAMESPACE, 'VoiceReceiptRejected', 'ReceiptNotFound');
    ...
    emitOutcomeMetric(METRIC_NAMESPACE, 'VoiceReceiptUpdated');
    return {
      statusCode: 200,
    ...
    logError('alerting.receipts.voice.failed', error, traceId, {
      dispatchId: body.dispatchId,
      memberId: body.memberId,
    });
    emitOutcomeMetric(METRIC_NAMESPACE, 'VoiceReceiptRejected', 'DynamoUnavailable');
