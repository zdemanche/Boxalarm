...
export const DEFAULT_CERT_EXPIRY_LEAD_DAYS = 30;
...
export async function readCertExpiryLeadDays(
  ...
  try {
    ...
    const leadDays = value?.certExpiryLeadDays;
    ...
    console.error(
      JSON.stringify({
        event: 'certificationExpiryScanner.config.read_failed',
        service: 'training',
