...
describe('statusChange real withAuthorization wiring (P5 regression)', () => {
  ...
  it('sends the correct actionType/actionId/resourceType/resourceId to Verified Permissions', async () => {
    ...
    expect(vpCall.input.action).toEqual({ actionType: 'MEMBER', actionId: 'UpdateMemberStatus' });
    expect(vpCall.input.resource).toEqual({ entityType: 'MEMBER', entityId: 'mbr-1' });
