const EXPORT_ROLES = new Set(['CHIEF', 'ADMIN']);
...
// server-side check on this endpoint, per architecture Session/reauth policy (:2621).
export function assertChiefOrAdmin(cognitoGroupsRaw: string): void {
  const groups = parseGroups(cognitoGroupsRaw);
  if (!groups.some((group) => EXPORT_ROLES.has(group))) {
