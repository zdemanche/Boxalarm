...
flowchart TB
    ...
    LOB --> LBUS --> LQ
    LBUS -->|"training.expiry.due · apparatus.test.due ·<br/>inventory.expiry.due · inventory.reorder.due ·<br/>scheduling.coverage_gap.detected"| NOTIF
    NOTIF --> PDB
...
> **`notification-service` (reconciled — CANONICAL).** The Events section routes `training.expiry.due` (N-5, renamed from `cert.expiry.due`), `apparatus.test.due`, `inventory.expiry.due` (N-5, renamed from `ppe.expiry.due`), `inventory.reorder.due` and `scheduling.coverage_gap.detected` to a "Notification Service" that previously existed in no service list. It is service 10 above. Its contract:
>
...
flowchart TB
  ...
  VoiceQ[[SQS voice-queue]] --> VoiceWorker[Voice Worker Lambda] --> VoiceVendor[("Voice vendor\n(distinct from SMS, TBD)")]
  ...
  Escalator[Escalation Scheduler\nEventBridge Scheduler\nvoice tier only — gated on no ack\nacross push AND sms at T+N] -->|"no ack on push AND sms\nat T+75s (N1.2: primary tier\nis parallel; voice is sole escalation)"| VoiceQ
...
> **Event naming (reconciled).** The Events section is the authority on event names and envelope shape, and its convention — lowercase dotted `<domain>.<entity>.<past-tense-verb>` — is canonical throughout this architecture. Where this Backend section used PascalCase shorthand, read it as the dotted name: `DispatchReceived` → `alerting.dispatch.received`, `ResponseConfirmed` → `alerting.response.confirmed`, `MemberUpdated` → `personnel.member.updated`, `CertExpiring` → `training.expiry.due`, `CheckCompleted` → `apparatus.check.completed`, `IncidentSubmitted` → `neris.incident.submitted`. Implementations MUST emit the dotted form; the PascalCase names appear nowhere in code, config, or EventBridge rule patterns.
>
> **`domain` is always the owning service's short name (N-5, decided).** `dispatch.alert.received` and `cert.expiry.due` violated this — the CAD-ingress event is `alerting-service`'s own (its bounded context is literally "Dispatch ingress"), and cert expiry is `training-service`'s (§1.1 service 6). Both are corrected throughout this document to `alerting.dispatch.received` and `training.expiry.due`; `ppe.expiry.due` is corrected to `inventory.expiry.due` on the same rule (`inventory-service` owns PPE per §1.1 service 9) — all three already disagreed with their own SNS topic names (`moonaan-prod-training-topic`, `moonaan-prod-inventory-topic`), which were the tell. **`scheduling.coverage_gap.detected` is deliberately left alone**: its topic (`moonaan-prod-scheduling-topic`) already agrees with its event name, so unlike the other three this isn't a self-contradicting mismatch — it's shift coverage living under its own domain rather than `personnel-service`'s, which may be intentional. Flagged, not changed.
...
| PUT | `/api/v1/apparatus/{unitId}/defects/{defectId}/status` | Resolve or reopen a defect (N-9, decided) — sets `status`, and `resolvedAt`/`resolvedBy` when transitioning to `RESOLVED` (F4.3, Data Model `DEFECT`) | Cognito(admin) |
...
| GET | `/api/v1/reporting/losap/year-end` | LOSAP year-end report (F8.2). **Sole owner (N-3, decided):** this endpoint was duplicated verbatim as `/api/v1/personnel/losap/year-end` with no F-number and no distinguishing scope — removed from `personnel-service`. `reporting-service` owns every named compliance/grant report (F8.1–F8.7) by charter (§7); `personnel-service` keeps only the per-member running total (`/api/v1/personnel/members/{memberId}/losap`, F2.4), which this report aggregates across the department. | Cognito(admin) |
...
| PUT | `/api/v1/inspections/{id}/violations/{code}` | Update one violation's status (N-9, decided) — addressed by its `code` within the inspection's `violations` list (Data Model `INSPECTION_RECORD`); a re-inspection recording the same code again is a new list entry on a new `INSPECTION_RECORD`, not an update to this one | Cognito(admin) |
...
| PUT | `/api/v1/inventory/consumables/{itemId}` | Adjust stock level / reorder threshold (N-9, decided) — sets `stockLevel`/`reorderThreshold` on `CONSUMABLE_STOCK` directly; no adjustment-history entity exists in the Data Model, so this is last-writer-wins like every other current-state PUT in this table, not an append-only ledger | Cognito(admin) |
...
**Endpoint count: 83** (67 business endpoints — net +2 from N-9's three new write endpoints minus N-3's duplicate LOSAP year-end removal — + the 16 `health/liveness`/`health/readiness` pairs §4.3 requires of every service, added to this table by M-15) across 9 services, plus 6 `notification-service` endpoints (4 from the canonical contract note in §1.1, plus its own health pair; NEW-9) = **89 across 10 services**.
...
sequenceDiagram
    ...
    CAD->>Fan: alerting.dispatch.received
...
flowchart LR
    ...
    CertScan[Cert Expiry Scanner<br/>scheduled] -->|training.expiry.due| T1 --> NotifSvc[Notification Service]
    ApTestScan[Apparatus Test Scanner<br/>scheduled] -->|apparatus.test.due| T2 --> NotifSvc
    PPEScan[PPE Expiry Scanner<br/>scheduled] -->|inventory.expiry.due| T3 --> NotifSvc
...
- `training.expiry.due` (N-5, renamed from `cert.expiry.due`) — `{memberId, certId, expiryDate, leadDays}`
- `apparatus.test.due` — `{apparatusId, testType, dueDate}`
- `inventory.expiry.due` (N-5, renamed from `ppe.expiry.due`) — `{memberId, ppeItemId, expiryDate}`
...
|---|---|---|---|
| `alerting.dispatch.received` (N-5, renamed from `dispatch.alert.received`) | Dispatch Ingress Port (adapter, vendor TBD) | Alert Fan-out Service | Direct invoke or dedicated SQS (ingress-specific; depends on adapter — see Open Questions) |
...
| `training.expiry.due` (N-5, renamed from `cert.expiry.due`) | Certification Expiry Scanner (daily scheduled Lambda) | Notification Service (member + training officer) | SNS `moonaan-prod-training-topic` → `training-notify-queue` |
...
| `inventory.expiry.due` (N-5, renamed from `ppe.expiry.due`) | PPE Expiry Scanner (daily scheduled Lambda) | Notification Service | SNS `moonaan-prod-inventory-topic` → `inventory-notify-queue` |
